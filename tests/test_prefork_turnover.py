...
def test_turnover_skipped_before_worker_setup(monkeypatch: pytest.MonkeyPatch) -> None:
    # No node name captured yet ⇒ the worker has not reached ``celeryd_after_setup``,
    # so its prefork pool has not forked: the children it forks next inherit THIS
    # (post-op) registry and none is stale — nothing to recycle.
...
def test_turnover_waits_for_worker_ready_then_recycles(monkeypatch: pytest.MonkeyPatch) -> None:
    # Node name captured but the pidbox is not consuming control commands yet: the
    # bootstrap child may have forked from the PRE-op registry, so the turnover must
    # NOT skip — it waits for the worker to start consuming, then recycles the pool.
    prefork._worker_ready.clear()
...
def test_turnover_raises_if_worker_never_starts_consuming(monkeypatch: pytest.MonkeyPatch) -> None:
    # Node name captured but the worker never begins consuming control commands: a
    # possibly-stale bootstrap child cannot be recycled, so the op must fail loudly
    # rather than reply ``applied`` behind a stale child.
    prefork._worker_ready.clear()
