...
A task run in a pool child after a mutating op is applied in the PARENT keeps
serving the OLD registry until the pool re-forks — a main-process-only observation
would pass while the child is stale, so the assertions here read back through a
pool child (``probe_registry`` returns the child pid + the registry that child was
forked with), proving the successor forces and confirms that turnover.

It runs a genuine Celery prefork worker (real ``fork``, real ``pool_restart``,
real ``stats`` turnover confirmation) against a Redis broker. The broker delivery
of the op itself lives in the skeleton's worker bus (exercised there); here the op
is applied in the parent and the successor's public entrypoint
(``_on_fleet_op_applied``) is driven, exactly as the bus drives it after an apply.

The test is skipped when no Redis broker is reachable — set ``TAI_TEST_CELERY_BROKER``
(default ``redis://localhost:6399/0``) to a running Redis to run it.
"""
