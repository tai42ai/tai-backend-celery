...
``tai42_backend_celery`` is on the allowlist. The rule (see the README): the shipped
package imports
...
The runtime walk imports ``tai42_backend_celery`` and every submodule in a fresh
subprocess, then inspects ``sys.modules``. Running it in a subprocess that
imports ONLY ``tai42_backend_celery`` means the assertion covers the SHIPPED package's
