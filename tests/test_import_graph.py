...
``tai_backend_celery`` is on the allowlist. The rule (see the README): the shipped
package imports
the Celery stack (celery / kombu / billiard / amqp / vine /
celery-redbeat / celery-pydantic / flower / redis), the shared platform
...
The runtime walk imports ``tai_backend_celery`` and every submodule in a fresh
subprocess, then inspects ``sys.modules``. Running it in a subprocess that
imports ONLY ``tai_backend_celery`` means the assertion covers the SHIPPED package's
...
PACKAGE = "tai_backend_celery"
ALLOWED_FIRST_PARTY = frozenset({PACKAGE, "tai_contract", "tai_kit"})

# Every third-party root the shipped ``tai_backend_celery`` graph pulls in -- the
...
def test_shipped_package_imports_only_allowlisted_roots() -> None:
    ...
    assert result.returncode == 0, (
        f"importing the shipped tai_backend_celery graph failed:\nstdout:\n{result.stdout}\nstderr:\n{result.stderr}"
    )

    offenders = [line for line in result.stdout.splitlines() if line]
    assert offenders == [], f"non-allowlisted roots in the tai_backend_celery module graph: {offenders}"
...
def test_shipped_sources_name_only_allowlisted_roots() -> None:
    ...
    assert offenders == {}, f"non-allowlisted import roots in the tai_backend_celery sources: {offenders}"
