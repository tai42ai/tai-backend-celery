...
``redbeat:{name}`` with ``definition``/``meta`` fields and the schedule zset
``redbeat::schedule`` whose members are the entry keys — ``redbeat:{name}``,
not bare names — and whose scores are next-run timestamps), reached through
the host's pooled Redis client. Every schedule tool therefore addresses the
zset by the entry key, and accepts ``name`` either bare or already prefixed.

``backend_list_schedules``, ``backend_delete_schedule``,
``backend_export_schedules``, and ``backend_import_schedules`` are the host's
scheduling marker tools; their names and semantics are load-bearing (the
schedules API and the backup round trip call them by name).
"""
...
def _redbeat_redis() -> AbstractAsyncContextManager[Any]:
    """The pooled async Redis client for the RedBeat store.

    Typed as yielding ``Any``: redis-py annotates each command as a
    sync/async union, which cannot be awaited as-is under the type checker.
    """
    return tai_app.clients.client_ctx(RedisClient, url=celery_settings().redbeat_redis_url)
...
async def _read_definition_and_membership(r: Any, schedule_key: str, entry_key: str) -> tuple[Any, Any]:
    ...
    server instant (a single MULTI/EXEC pipeline).

    The pair drives the listing tools' dangling-member check, and it must be
    atomic: read separately, a whole-entry delete plus an atomic re-create
    (RedBeat's ``save``) landing between the two commands would show "no
    definition" next to "member present" — the corrupt dangling state — even
    though the store never actually passed through it (every deleter removes
    the zset member before the hash, and ``save`` writes both in one
    transaction).
    """
...
async def backend_delete_schedule(name: str) -> dict[str, Any]:
    ...
    async with _redbeat_redis() as r:
        ...
        # Remove the zset member first, then the entry hash — RedBeat's own
        # delete order. The reverse would transiently leave a dangling zset
        # member (a member with no backing hash), the corrupt state the
        # listing tools raise on and Beat silently drops.
        ...
        for member, score in items:
            ...
            if not definition:
                if member_score is None:
                    # Member and hash are both gone: the entry was deleted
                    # concurrently after the zrange snapshot.
                    continue
