...
Each factory receives ``(func, name, description)`` and returns a NEW-named
callable the host binds as a branch tool alongside the original:

* ``<tool>_sync_task`` — dispatch to a Celery worker and block for the result
  (up to ``CELERY_TASK_TIMEOUT`` seconds).
* ``<tool>_async_task`` — dispatch and return ``{"task_id", "status"}``
  immediately.
* ``<tool>_schedule_task`` — persist a RedBeat entry that runs the tool
  periodically (interval seconds or crontab).

The branch signatures extend the wrapped tool's own parameters with the Celery
task options (queue/countdown/priority/... plus ``callback_kwargs`` to chain a
follow-up tool) or the schedule options, built with makefun so the composed
signature is concrete. The wrapped tool itself is NOT called here — dispatch
injects the tool name into the kwargs and the worker-side ``tool_execution``
task runs it through the host handle.
"""
...
def sync_task(func: Callable[..., Any], name: str, description: str) -> Callable[..., Any]:
    ...
    async def func_impl(*args: Any, **kwargs: Any) -> Any:
        ...
        try:
            # A failed task re-raises its own exception here (``result.get``
            # revives the original class) — it is never wrapped.
